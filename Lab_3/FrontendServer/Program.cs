    ...
    class Program
    {
        ...
        {   switch (request.Url.AbsolutePath.ToLowerInvariant())
            {   //статические страницы
                ...
                #region обращения к photos server
                case "/photoscontrol-submit":
                                ...
                                {   var rc = new RestClient(ServerAddress.PhotosServerAddress).Execute(new RestRequest("photo/" + pars["id"], Method.GET));
                                    ...
                                    return
                                        Pages.Table.Replace("CONTENT", 
                                                            ...
                                                            obj.id, obj.owner, obj.uploaded));
                                }
                            case "CREATE": //POST
                                {   if (pars["uploaded"].Trim().Length == 0)
                                    ...
                                    rq.AddParameter("uploaded", pars["uploaded"], ParameterType.GetOrPost);
                ...
                case "/photos": //часть результатов может возвращать без авторизции
                    {   if (is_authorized(request, response))
                        {   return Pages.Table.Replace("CONTENT",
                                JsonConvert.DeserializeObject<List<Photo>>(
                                    ...
                                    .Select(p => string.Format("<th class=\"tg-031e\">{0}</th><th class=\"tg-031e\">{1}</th><th class=\"tg-031e\">{2}</th>", p.id, p.owner, p.uploaded))
                ...
                case "/clientscontrol-submit":
                    {
                        ...
                        {   case "READ": //GET
                                {   var rc = new RestClient(ServerAddress.ClientsServerAddress).Execute(new RestRequest("client/" + pars["id"], Method.GET));
                                    ...
                                    return
                                        Pages.Table.Replace("CONTENT",
                                                            ...
                                                            obj.id, obj.name, obj.address, obj.email));

                                }
                            case "CREATE": //POST
                                {   if (pars["name"].Trim().Length == 0)
                                        return response.SetStatusWithMessage(HttpStatusCode.BadRequest, "name, email & address are required");
                                    var rq = new RestRequest("client", Method.POST);
                                    rq.AddParameter("name", pars["name"], ParameterType.GetOrPost);
                                    rq.AddParameter("email", pars["email"], ParameterType.GetOrPost);
                            ...
                            case "UPDATE": //PUT
                                {   if (pars["name"].Trim().Length == 0)
                                        return response.SetStatusWithMessage(HttpStatusCode.BadRequest, "name, email or address is required");
                ...
                case "/register-submit":
                    {   var pars = request.GetPOSTParams();
                        var rq = new RestRequest("register", Method.POST);
