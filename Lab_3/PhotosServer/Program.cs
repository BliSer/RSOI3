...
namespace PhotosServer
{
    class Program
    {
        static string photosFile = "../../../resources/db/photos";
...
/item GET [owner] [uploaded]  - возвращает список фотографий
/photo/id GET, DELETE
...
Все параметры являются подстроками для поиска.";
        ...
        {   switch (request.Url.AbsolutePath)
            {
                case "/item": //возвращает список работников
                    {   var pars = request.QueryString;
                        return JsonConvert.SerializeObject(photos.Where(p =>
                               p.owner.ContainsAny(pars.GetVals("owner")) && p.uploaded.ContainsAny(pars.GetVals("uploaded"))));
                    }
                default: // photo/
                                ...
                                {   case "GET":
                                    ...
                                    case "POST":
                                        {   if (id != -1)
                                            ...
                                            if (pars.Keys.Contains("owner") && pars.Keys.Contains("uploaded"))
                                            {   photos.Add(new Photo(GetNewID(), pars["uploaded"], pars["owner"]));
                                                return response.SetStatusWithMessage(HttpStatusCode.Created, "Successfully created");
                                            }
                                            else
                                                return response.SetStatusWithMessage(HttpStatusCode.BadRequest, "Request should contain POST owner, upload date parameters");
                                        }
                                    case "PUT":
                                        {   var pars = request.GetPOSTParams();
                                            if (record != null)
                                            {   if (pars.ContainsKey("owner"))
                                                ...
                                                if (pars.ContainsKey("uploaded"))
                                                    record.uploaded = pars["uploaded"];
