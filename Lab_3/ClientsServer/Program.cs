...
/item GET [name] [address] [email] - возвращает список работников
...
Все параметры являются подстроками для поиска.";
        ...
        {   switch (request.Url.AbsolutePath)
            {   case "/item": //возвращает список пользователей
                    {   var pars = request.QueryString;
                        return JsonConvert.SerializeObject(clients.Where(c =>
                               ...
                               && c.email.ContainsAny(pars.GetVals("email"))));
                    }
                default: // /client
                                ...
                                case "POST":
                                    {   if (id != -1)
                                        ...
                                        if (pars.Keys.Contains("name") && pars.Keys.Contains("email") && pars.Keys.Contains("address"))
                                        {
                                            ...
                                            return response.SetStatusWithMessage(HttpStatusCode.BadRequest, "Request should contain POST name, email, address parameters");
                                    }
                                case "PUT":
                                    {   var pars = request.GetPOSTParams();
                                        if (record != null)
